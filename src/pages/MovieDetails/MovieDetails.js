...
const MovieDetails = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://localhost:7230/api/Movie/${movieId}`, { headers })
      .then((res) => res.json())
      .then((data) => {
        setMovie(data);
        ...
        Promise.all(
          data.genres.map((genreId) =>
            fetch(`https://localhost:7230/api/Genres/${genreId}`)
              .then((res) => res.json())
        ...
        Promise.all(
          data.actors.map((actorId) =>
            fetch(`https://localhost:7230/api/Actors/${actorId}`)
              .then((res) => res.json())
              .then((actor) => actor.name)
          )
        ).then(setActors);

        fetch(`https://localhost:7230/api/Directors/${data.directorId}`)
          .then((res) => res.json())
